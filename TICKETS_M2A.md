...
## Part B 底栏与「记一笔」重设计（用户指令②，须先拍板）
- **规格来源**：用户在 `design/bottom_bar_preview.html` 选定方向（A / B / C）→ 管理层冻结进 `DESIGN_MAIN.md` 新章节（含尺寸/间距/token 引用/三 tab 视觉）→ 才可施工
- **硬约束**：**禁投影 / 禁发光 / 禁循环动画**；渐变填充**只有在冻结稿明确保留时才允许**（默认按「纯色金或描边」实现，理由：用户原话「丑」= 渐变实心条）；色值/字号/间距**只准引用 `tokens.dart`**；触控 ≥48；`reduce-motion` 全退化；衬线仅可用于品牌时刻
- **必须覆盖的态**：分析（选中 + 有 CTA）/ 资产（选中 + 无 CTA）/ 统计（选中 + 无 CTA）/ 未选中 tab / 按下态 / reduce-motion
