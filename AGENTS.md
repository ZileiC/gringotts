...
- 深色唯一主题；黑金体系（近黑底 + 香槟金 accent）；**禁投影、禁渐变填充、禁发光、禁循环动画**（例外仅金渐变章程允许处：主页 Hero 额度 / 资产净值 / 底栏「记一笔」主按钮 / 饼图金阶——见 `DESIGN_MAIN.md` §7）
- 衬线（Playfair）仅限品牌时刻：主页 Hero 额度 / 资产净值 / wordmark / 快记金额行 / 键面数字（符号除外）
- 结构：底栏三 tab 平级（分析/资产/统计，切换不压栈）；「记一笔」主按钮压栈进快记页（分析页下一级）；明细页挂统计页，时间优先（月→日→条目）
