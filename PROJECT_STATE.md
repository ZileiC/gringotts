...
## 状态（2026-09-15，**T-21 规格已冻结，可派施工** —— M1 系列收官票）
- **当前票 = T-21（P1）**：统计页图表重整 + 收入/存款语义。**规格 = `DESIGN_MAIN.md` §11（用户 2026-09-15 拍板）**：
  - **两张图都做**：图 1 = 每日支出柱 + 额度金虚线 + 超额红段 + 临时收入绿点；图 2 = 双线趋势（支出 vs 收入，收入含保底均摊）
  - **纵轴必须有金额刻度**、**横轴标签必须对齐数据点**（旧实现两条都错，诊断见 T-21）
  - **日视图 = 所选月整月**（不再固定 7 天）；**统计页加月份按钮并与主页/明细联动**
  - **计划存款 → 资产**：月末提示卡 + 手动确认，「存进资产」生成存款类资产、「没攒够」零落库（含 V3→V4 迁移）
- **派工 prompt**：`SESSION_PROMPTS.md` §C（含熔断规则，**直接可派**）
- **已关闭**：M2.0 前置波（T-10a→T-14）+ T-14b（导航语义 + 底栏/顶栏重设计）；T-14b APK 待你真机目测
- **质量基线**：207 单测全绿（analyze 零问题）；回归 14/14 脚本；dev 库跑后 live 全 0
- **交付物**：桌面 `gringotts-T14b-release.apk`（md5 `8cf25112990f32830d371bc6eedb83ae`）；**T-21 完成出 M1 收官包** `gringotts-T21-release.apk`；仓库 github.com/jharayden/gringotts（private）
