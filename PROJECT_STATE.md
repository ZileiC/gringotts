...
## 项目状态
- **阶段**：**M1.0 已实机验收（用户 2026-09-11 测试完成）→ M2.0 前置波施工中**（T-10 预算引擎+新主页 / T-11 快记重设计★等选型 / T-12 明细页 / T-13 字体回归+M1.x）
- **用户实机反馈四条**（2026-09-11）：①主页应为分析/指导页（预算→每天可花多少钱），快记不该是主页 ②快记页设计丑、AI 味重，整体重做（先看 2–3 方向）③资产页很好但**说好的字体没了**（管理层误读导致，§6 更正）④已确认收支无法查看/修改，需明细页
- **本轮用户裁决**：预算＝每月手填（收入+计划存款，按月记住可改）· 额度＝固定基准+实时剩余**两个都要** · 快记页整体推翻先看方向 · 明细页全字段可改+删除墓碑
- **技术栈**：Flutter + Drift (SQLite) + Riverpod；schemaVersion = 2（→3 由 T-10 迁移）
- **主端**：Android 手机核心（APK 直装）；Windows = 开发预览 + 后期 AI 分析/备份工作台
- **仓库**：github.com/jharayden/gringotts（private）；M1.0 交付 APK：桌面 `gringotts-T09E-release.apk`（md5 `5c26de35…`）
- **版本语义**：M1.0 已完成；M2.0 = AI 上线（本波为其前置：主页/AI 落点先建）
- **质量基线**：116 单测全绿 + 9 个 integration 全绿 + profile 0 missed frames + tokens.dart 单点色值
- **最后更新**：2026-09-11（M2.0 前置波派工：TICKETS_M2A.md + DESIGN_MAIN.md + 五屏选型稿）
