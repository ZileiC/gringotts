...
## 开放项
1. **T-12c（P1，导航补口）**：主页底栏补「资产 / 统计」入口（页面本体完好，缺的只是入口；根因=管理层挂账未落工单）
2. **T-12b（P1）**：`updateFields` 无条件写 merchant/note → 回顾页批量补类别会清掉草稿商户/备注
3. T-13：资产字体回归 / 照片孤儿文件 GC / 启动画面 wordmark 重复（待用户裁决）/ M1.x 清账
4. 备用裁决池：截屏记账（M1.0 实测后定生死）、语音记账、剪贴板捕获
5. ✅ `AGENTS.md` 已压缩（6.4KB → 2.5KB，用户 2026-09-12 授权后写入）
6. **新流程条款**：验收记录里的挂账项，必须写入下一张工单才算出账（本轮 T-10b 遗留未落 T-12 工单导致用户实机踩空）
