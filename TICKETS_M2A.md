...
- **改动 3：键盘重设计**（DESIGN_MAIN §4.2 规格，验收逐条对照）：键宽 101 × 高 64（横 gap 16 / 纵 gap 12）· 圆角 **18** · **数字键 = `elevated` 填充无描边；功能键（`.` `⌫`）= 透明 + hairline**（材质分层打破等权单调）· 键面数字 **24px / w500 / 系统 sans + tabular**（**不可用衬线**：apple-design §15 默认系统字体，小字号高对比衬线损可读性）· **金额行用 Playfair 600 / 44px**（用户点名：净值同款字体、字号小于 hero）· 键区上下各留 ≥12px 留白
