...
## Part B 底栏与「记一笔」重设计（用户指令②，**先拍板才可施工**）
- **规格来源**：用户在 `design/bottom_bar_preview_v2.html` 选定组合（tab 态 T1/T2/T3 × 位置 P1/P2/P3 × 按钮 D1/D2/D3，如 `T1/P1/D1`）→ 管理层写成 `DESIGN_MAIN.md` §8 终稿 → 才可施工。**v1 三案（底栏上方通栏 / 卡片内 / 底栏居中）已被用户否决，勿复用**
- **已冻结不变量**：底栏**只放三个 tab**；「记一笔」仅属分析页；自绘 1.25px 线稿图标（分析＝上升折线+终点圆 / 资产＝层叠双框 / 统计＝三根不等高竖线），不用 Material 通用件；底栏总高恒定
- **硬约束**：**禁投影 / 禁发光 / 禁循环动画 / 禁渐变填充**；色值/字号/间距只准引用 `tokens.dart`；触控 ≥48；`reduce-motion` 全退化（滚动收缩、按下缩放等动效一并退化）；衬线仅限品牌时刻
- **必须覆盖的态**：分析（选中 + 有「记一笔」）/ 资产（选中 + 无）/ 统计（选中 + 无）/ 未选中 tab / 按下态 / 滚动中（若选 P1 的收缩行为）/ reduce-motion
