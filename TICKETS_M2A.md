...
### Part D 月份选择改「按钮 + 月历弹窗」（用户 2026-09-12 追加）
- 主页左上角月份标题 → **月份按钮**（`2026 年 9 月 ▾`）；点击弹出**月历 sheet**（完整规格见 `DESIGN_MAIN.md` §3 第 1 条：年份 `‹ 2026 ›` + 月份 3×4 宫格、当前月金边选中、未来月灰显禁用、选中即切换、与类别网格同语言且禁渐变投影）
- **删除**主页右上方的左右箭头（两个 `icobtn`）；右上角只留预算设置 `⚙`
- 验收：点按钮 → sheet 弹出 → 选月 → 主页数据随月切换（状态断言）+ 未来月不可点 + 箭头已移除（无残余 key）

**验收（全票）**：① 三 tab 平级切换 + 「记一笔」压栈进快记页 + 返回回分析页（导航断言）② 统计页可达明细页 ③ **快记一笔后立即可在明细/统计/主页额度中看到**（不再需要确认）④ 回顾页/批量补类别/徽章相关代码与测试全部清除（无死引用）⑤ `updateFields` 无陷阱残留 ⑥ **月份按钮 + 月历弹窗可用、箭头已移除** ⑦ analyze 零错误 + test 全绿（数量变化需说明哪些测试被删）⑧ Windows 实跑帧（Dart PNG + md5 唯一）⑨ **完成后重建 release APK 交付用户做全流程真机测试**
