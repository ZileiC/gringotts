...
- **五层验收**：
  1. **记录核对** ✓ 6 个提交齐；变更面 = `statistics_service.dart`(+185) / `stats_page.dart`(+908) / 新增 `ui/month_sheet.dart`(299) + `services/savings_plan.dart`(94) / `app_database.dart`(+27，`.g.dart` 重生) / `models.dart` / 两个仓库 / `home_page.dart`（-404 重构：抽走月历 sheet）/ `ledger_page.dart` / `app.dart`(+16) / `asset_detail_page.dart`(+3) / 测试 +30（237 total）；收工提交含 WORKLOG + evidence（帧清单/APK 记录/analyze 与 test 日志/3 份 regression 日志）
  2. **独立复验** ✓（管理层亲跑）`flutter analyze` → **No issues found**；`flutter test` → **All tests passed (237)**（基线 207，+30）—— 与申报逐位一致
  3. **源码级审查**（逐条对 `DESIGN_MAIN §11`）✓ ① **旧 7 桶 `dailyTrend` 全仓零命中**（诊断①根除）② 新纯函数：`monthDayLabels` / `monthDays(month)`（28/29/30/31 桶）/ `incomeBaselinePerDay(BudgetMonth?)`（÷当月天数，无预算或墓碑 → null）/ `monthlyTrend(year)` / `yearlyTrend` / `spendBars` + `SpendBar` ③ 图 1 = `_SpendChartCard`（key `stats_spend_chart`）：额度内 `elevated`+hairline / 超额段 `semanticExpense` / 额度线 `goldAccent` 1px `dashArray [4,3]` / 临时收入 4px `semanticIncome` 圆点 + 数字标签 / 无预算不画线并出「先设置本月预算」④ **轴修正到位**：底轴 `interval: 1` 且 x 只取整数 index，源码注释明写「T-21 诊断 2 的 `length/6` 被禁用」⇒ **标签滑移的根因已消灭**；左轴 `_leftAxis(maxY)` `showTitles: true` + `interval = maxY/3`（0 / ⅓ / ⅔ / max 四档，千分位 + tabular）⑤ 图 2 = `_TrendChart`（key `stats_trend_card`）：收入线用 `PeriodPoint.incomeLineCents`（保底均摊 + 临时收入）；**缩放 `max(支出, 保底均摊) × 1.15`，临时收入不入缩放**、改虚线引线 + 圆点 + 标签（注释写明理由）⑥ 月份：`selectedMonthProvider`（`app.dart`）被 `home_page` / `ledger_page` / `stats_page` 三处 watch ⇒ 一处切换三处同步；`_MonthSheet` 抽为共享 `MonthSheet` + `showMonthSheet`，统计页 `stats_month_button` / `stats_month_sheet` 复用同一 sheet ⑦ 迁移：`schemaVersion 4`，`from < 3` 与 `from < 4` 分段守卫 + `addColumn`（`budget_months.savingsConfirmedAt/SkippedAt`、`assets.note`，全 nullable）⑧ 存款：`SavingsPlan` 纯规则 + `SavingsPlanService.confirm/skip`（confirm 先 `findPlannedSavingsAsset` 再建 = **幂等**）、`home_savings_card` 两键 + 最后 3 天文案 ⑨ **派生值不落库** ✓ 新增落库字段仅两个确认时间戳 + `assets.note`（生成资产的备注）
  4. **独立 integration** ✓（管理层亲跑三个）`t05_stats_test` / `t10b_home_test` / `t13b_full_chain_test` **全部 exit=0**；t13b 打印 `export files=2 bom=true csv_merchant=true json_asset=true schema=4`（schema 随 V4 变 4，预期）、`teardown live_tx=0 live_assets=0 exported_files_removed=2`
     - **真库迁移独立复核** ✓ 直读 dev 库：`user_version = 4`、`budget_months` 含 `savings_confirmed_at`/`savings_skipped_at`、`assets` 含 `note`、业务表 **live 全 0**（跑后无残留）
  5. **UI 目检 + 数学复核** ✓ t05 日视图帧：顶栏「统计 + `2026 年 9 月 ▾` + 明细 ›」；日/月/年 chips（日选中金字 + 勾选）；**两张图结构到位**（图 1「每日支出 · 额度对照」在上、图 2「收支趋势」在下）；空库运行 ⇒ **正确降级**：不画额度线 + 图上方「先设置本月预算」+ 空态「这个周期还没有记录」（正是 §11.2 的无预算分支）。数学：底轴 `interval = 1` 与「标签 x = index」互证；左轴 `interval = maxY/3` ⇒ 四档与 §11.2 一致
- **⚠️ 证据观察（P3，不阻塞验收；已立 T-22）**：① `evidence/t21/frame_md5.txt` 的 `[t05_stats_test]` 段落**空**——t05 的 4 帧确实生成（仓库根 `.t05_*.png`，本地、已 gitignore）但未登记 md5 ② 本票全部帧均为**空库/无预算**运行 ⇒ 没有一张「有预算 + 有流水」的**填充态**统计页帧。**填充态正确性由结构化单测承担**（`stats_page_test` 270 行：柱数 = 当月天数且 x = index、左轴 `showTitles` 且 `interval = maxY/3`、底轴 `interval = 1`、额度线 y = 日额度且 `dash[4,3]`、超额红段、收入线 = 保底均摊非 0、`maxY = max(支出,均摊) × 1.15` 且尖峰不入缩放、无预算说明、空态）—— 符合本项目「确定性单测优先于像素帧」的证据口径；两条已入 **T-22**
- **熔断纪律执行良好**：5 个 Part 各自 WIP 提交（掉线可续）；**integration 只跑了被点名的 3 个**（未全量重跑）；无一次 harness 内部错误；未降断言 / 未 skip / 未删测试
- **新增环境事实**：该 harness 的**命令通道会吞掉非 CJK 非 ASCII 符号**（U+00A5 等实测被吞），执行层改用 Dart 转义 `\u00a5` 写入源码 —— 已记入 memory，后续派工 prompt 需提醒
- **交付**：`C:\Users\JHarayden\Desktop\gringotts-T21-release.apk`（66,504,468 字节，md5 **`8cd0d04b0aaad7f4df7b9bab00c21d87`**；M1 系列收官包，比 T-14b 包 +589,824 字节 = 两张新图 + 共享月历 + 存款卡 + V4 schema）—— **待用户真机目测**
- **结论：T-21 ✅ 验收通过 → M1 系列（含全部追加修正）全部关闭**。M2.0 正式波仍按用户安排暂缓
...
- **Part 6 完成（收尾）**：按令只跑受影响的 3 个 integration（`t05_stats_test` / `t10b_home_test` / `t13b_full_chain_test`）**全部 exit=0**（日志 `evidence/t21/regression/*.t21.log`）；`flutter analyze` -> **No issues found**；`flutter test` -> **237 passed**（基线 207，+30）；`flutter build apk --release` exit=0（Gradle 86.8s / 63.4MB）-> 桌面 **`gringotts-T21-release.apk`**：`bytes=66504468`、`md5=8cd0d04b0aaad7f4df7b9bab00c21d87`（`evidence/t21/apk_md5.txt` + `apk_build.log.txt`）；帧 md5 清单 `evidence/t21/frame_md5.txt`（3 脚本，PNG 本地）
- **真库迁移实证**：dev 库起手 `user_version=3`（无新列）-> 首个 integration 打开后自动升到 **`user_version=4`**（`budget_months` 增 `savings_confirmed_at`/`savings_skipped_at`，`assets` 增 `note`），跑后 live tx / asset / budget 全 0；迁移前已备份 `gringotts.sqlite.pre-clean-20260915-132032.bak`（`evidence/t21/dev_db_cleanup.json`）
- **t13b 导出 `schema=4`**：export 的 `version` 取自 `db.schemaVersion`（`export_service.dart:85`），随 V3->V4 变 4，属预期；t05/t10b/t13b 帧 md5 变化均可解释（统计页两张新图、分析页新增存款卡、字体/布局未变）
- **遗留**：无阻塞项。备注：本 session 命令通道会吞非 CJK 非 ASCII 符号（U+00A5 等），新写源码/测试一律用 Dart 转义；`pubspec.lock` 因 build_runner 运行有版本记录变化，一并提交
