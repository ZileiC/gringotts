...
| T-14b | **导航语义修正 + 底栏/顶栏重设计**（记一笔仅属分析页、移到顶栏为只有加号的圆环键；底栏只放三 tab + 自绘线稿图标 + 金细线滑动；MiSans 引入）| `14c942e`+`0cc6190`+`b5b4373` |
| T-21 | **统计页图表重整 + 收入/存款语义**（两张图：柱+额度线 / 双线含保底均摊；月份三页联动；计划存款→资产 V3→V4）**M1 收官** | `d96da7d`+`0f138fa`+`5902382`+`30afb1e`+`64b1966`+`38235eb` |
...
# T-14b（M2 前置修正票，P1：导航语义 + 底栏重设计）—— ✅ **已验收通过（2026-09-15，`14c942e`+`0cc6190`+`b5b4373`）**，APK 已交付待真机目测
...
# T-21（M1 系列最后一票：统计页图表重整 + 收入/存款语义）—— ✅ **已验收通过（2026-09-15，`d96da7d`+`0f138fa`+`5902382`+`30afb1e`+`64b1966`+`38235eb`）**，M1 收官包已交付

> 用户四条反馈 + 一个语义追问；**管理层已源码级核实：四条全部成立**（不是观感问题，是逻辑与坐标错误）。
> **规格已冻结 = `DESIGN_MAIN.md` §11**（用户 2026-09-15 拍板：**A 与 B 两张图都做** / 月份按钮并与主页·明细联动 / 存款转资产＝月末提示卡＋手动确认）→ 可直接派工。

## 诊断（管理层核过，含文件行号）
1. **日视图只有 7 天**：`StatisticsService.dailyTrend`（`statistics_service.dart:141-154`）硬编码 7 个桶（6 天前…今天），与"月"账期脱节
2. **横轴与数据点错位**：`stats_page.dart:256` 用 `interval: points.length / 6`（7 点 ⇒ **1.1667**），fl_chart 在 x=0/1.167/2.333… 处画标签并 `value.toInt()` 取名 ⇒ **标签整体滑移**，不落在柱/点上
3. **纵轴无任何金额刻度**：`stats_page.dart:248-250` `leftTitles: SideTitles(showTitles: false)` ⇒ 只有网格线没有数字
4. **"双线"实际只有一条有量**：代码确有两条 `LineChartBarData`（支出 `semanticExpense` / 收入 `semanticIncome`，`stats_page.dart:271-291`），但**收入序列只取 `type=income` 的流水**；用户心中的"收入"是主页手填的**月度预算**（`budget_months`，非流水）⇒ 收入线恒 0、贴 X 轴
5. **预算与存款完全未进图表**，且「计划存款达成」**没有任何转为资产的出口**

## 语义（管理层提案 —— 待用户拍板）
- **收入分两类**：**保底收入**＝主页手填月度收入（覆盖整月）→ 按当月天数**均摊为"每天可花额度"**（**派生值，不落库**，符合数据铁律「派生值不落库」）；**临时收入**＝快记 `type=income` 的流水 → 计入发生日
- **图表必须同时表达**：① 实际支出 ② 可花额度（日视图为水平基线）③ 临时收入（点/柱标记）
- **日视图 = 所选月份整月**（28–31 天），横轴标签**逐一对齐数据点**（整数 interval），纵轴**必须有金额刻度**
- **月份联动**：统计页顶部加与主页/明细同款「月份按钮」；日＝该月每天 / 月＝该年 12 个月 / 年＝全部年份（三页同一"当前月"）
- **计划存款 → 资产**：月末出现确认入口，一键生成一条**存款类资产**；**必须用户确认才落库**（不自动写库）

## 待用户拍板（见设计稿）
① 日视图形态：柱＋额度红线（管理层推荐）/ 双线 / 混合（日柱、月年双线）
② 收入呈现：额度基线＋临时收入绿点（推荐）/ 双线（保底均摊合成一条绿线）
③ 统计页月份按钮：加并与主页/明细联动（推荐）/ 独立 / 不加
④ 存款转资产：月末提示卡＋手动确认（推荐）/ 自动落库 / 只提醒

## 验收（拍板后细化，基线要求）
① 日视图覆盖所选月**全部天数**；横轴标签数 = 天数且每个标签 x 对齐对应 index（结构化断言，非像素）
② **纵轴有金额刻度**（左轴标题存在，首/末值与 maxY 口径一致）
③ 收入呈现正确：注入「月度预算 + 一笔临时收入」后，额度基线与临时收入标记**都在图上**（结构化断言）
④ 统计页月份切换与主页/明细一致（状态断言）
⑤ 存款确认流：**未确认时零落库**；确认后资产页出现该资产（金额/日期/来源断言），且该资产与手记资产同一套模型（可编辑/可卖/CPD 照算）
⑥ 空态 / 单点 / 极值（一个月仅一笔）不崩；`analyze` 零错 + 207+ 单测全绿；帧 md5 唯一；**本票末出 APK**（M1 系列收官包）

---

## T-22（M1 收尾补遗，P3，微票；可与下一票合并执行）
> 来源：T-21 验收时的两条证据观察（管理层 2026-09-15），不影响功能，只补证据完整性。
1. **补 `evidence/t21/frame_md5.txt` 的 `[t05_stats_test]` 段落**：t05 的 4 帧确实生成（仓库根 `.t05_state1_stats_daily.png` / `state2_stats_monthly` / `state3_stats_yearly` / `state3b_category_pie`，本地、已 gitignore），把它们的 md5 登记进清单（与 t10b/t13b 段落同格式）
2. **补一张「填充态」统计页帧**：播种「一个月的预算 + 若干笔支出 + 1 笔临时收入」后截统计页日视图（证明金虚线额度线、超额红段、临时收入绿点、双线实际观感），帧 md5 入库、PNG 本地；**只加/改证据脚本，不改产品代码**
**验收**：① t05 段 md5 补齐 ② 填充态帧 + md5 入库（并与 `DESIGN_MAIN §11` 逐项对照可行）③ `analyze` 零错 + `test` 仍 237 全绿 ④ 收工三连
