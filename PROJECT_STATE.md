...
- **下一票**：**T-22（P3 微票，补证据）** —— t05 帧 md5 登记 + 一张填充态统计页帧；可与下一票合并执行
- **你的真机验收清单（建议顺序）**：统计页日视图看「金虚线额度线 + 超额红段 + 12 号绿点 + 双线不再贴轴」→ 点月份按钮切月，看主页/明细/统计三页同步 → 月末卡片点「存进资产」看资产页多出存款条目 → 月/年视图两张图
