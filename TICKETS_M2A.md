...
## T-12c（导航补口，P1）：主页补 资产/统计 入口
**问题（用户实机测试发现）**：T-10b 的 IA 翻转后，「资产」「统计」的入口只剩快记页顶栏，**主页（启动页）够不到它们**——用户观感等同「这两个页面没了」。页面本体完好（`assets_page.dart` / `asset_detail_page.dart` / `stats_page.dart` 均未被删改，最后改动分别在 T-09D / T-09D / T-10b）；缺的只是入口。
**根因（管理层责任）**：T-10b 验收记录里的遗留第 2 项「把 资产/统计 收进主页入口体系」**未落到 T-12 工单**，挂账落空。
**改动**：主页底栏由 2 入口扩为 4 入口 —— `记一笔`（金渐变实心，flex 2，保持主 CTA）＋ `明细`（保留「今日 N 笔待完善」徽章）＋ **`资产`**（key `home_assets_cta`）＋ **`统计`**（key `home_stats_cta`），三个次级入口各 flex 1、金描边/文字样式，级距与现有底栏一致。
**保留**：快记页顶栏的 回顾/资产/统计 三入口**不动**（t09 系列测试依赖）。
**若底栏显挤**：备选＝把 资产/统计 放主页顶栏图标位（与 ‹ › ⧉ 并列），并在 WORKLOG 说明所选方案。
**验收**：主页一键直达 资产 / 统计（导航断言）；明细徽章仍工作；`home_assets_cta`/`home_stats_cta` 单测；analyze 零错误 + test 全绿 + Windows 实跑帧（Dart PNG + md5 唯一）。
**连带**：完成后**重建 release APK** 交付用户做全流程真机测试（此前 T-11 的 APK 不含 T-12 明细页，也带此导航缺口）。
